...
export default function SummaryComponent({
  ...
  const dateEndpoints = dynamicEndpoints.filter((e) =>
    e.path.includes("YYYY-MM-DD"),
  );
  return (
    <div
      key={dateStr}
      ...
      {log.scope === SummaryScopeTypes.Day && (
        <div className="flex gap-3 p-1">
          {dateEndpoints.map((endpoint) => {
            ...
            const charCount =
              endpoint.path === "/YYYY-MM-DD"
                ? log.keylogCharCount
                : endpoint.path === "/YYYY-MM-DD/screenshots/summaries"
                  ? log.screenshotSummaryCharCount
                  : undefined;
            const tokenStr =
              charCount !== undefined
                ? ` (${formatTokenCount(charCount)})`
                : "";
            return (
              <button
                key={endpoint.path}
