  ...
  return (
    ...
    >
      <div className="mb-1 font-bold flex items-center">
        ...
        {log.scope === SummaryScopeTypes.Day && log.screenshots.length > 0 && (
          <button
            className={
              (log.loading
                ? "text-gray-400 cursor-not-allowed "
                : "text-gray-600 hover:text-green-600 ") +
              "group relative ml-1 p-1"
            }
            onClick={() => window.userData.regenerateScreenshotSummaries(log.screenshots.map(s => s.imagePath))}
            disabled={log.loading}
          >
            <span className="absolute right-full mr-1 top-1/2 -translate-y-1/2 whitespace-nowrap text-xs bg-gray-800 text-white px-2 py-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none">
              Regenerate {log.screenshots.length} screenshot summaries
            </span>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
