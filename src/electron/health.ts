...
function parseLocalTimestampMsFromScreenshotSummaryFilename(fileName: string): number | null {
  const match = fileName.match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2})_(\d{2})_(\d{2})/);
...
export async function scanScreenshotSummaryMinuteSet({
  ...
  for (const dayDate of dayDates) {
    ...
    for (const entryName of entries) {
      const isJson = entryName.endsWith(".json") || entryName.endsWith(".json.crypt");
      if (!isJson) continue;
      const timestampMs = parseLocalTimestampMsFromScreenshotSummaryFilename(entryName);
      if (timestampMs === null) continue;
      if (timestampMs < sinceTimestampMs || timestampMs > nowTimestampMs) continue;
      minutes.add(minuteBucketFromTimestampMs(timestampMs));
    }
...
export function sqlLogitemsToMinuteSet(
  logitems: SqlLogitem[],
  sinceTimestampMs: number,
  nowTimestampMs: number,
): Set<number> {
  ...
  for (const logitem of logitems) {
    if (logitem.timestamp < sinceTimestampMs || logitem.timestamp > nowTimestampMs) continue;
    minutes.add(minuteBucketFromTimestampMs(logitem.timestamp));
  }
  return minutes;
}
...
export function renderHealthHtml({
  ...
  for (const minuteBucket of minuteSlots) {
    ...
    if (dateTitle !== lastDateTitle) {
      ...
      rows += `<div style="display:flex;flex-direction:row;align-items:flex-end;gap:4px;height:16px">` +
    ...
    const sqlColor = sqlMinutes.has(minuteBucket) ? "green" : "black";
    rows += `<div class="healthRow">` +
      ...
      `<div style="height:1px;width:25vw;background:${sqlColor}"></div>` +
      `</div>`;
  }
  ...
  return (
    ...
    `<div style="font-size:10px;width:25vw">sql</div>` +
    `</div>` +
    rows +
    `</body>` +
    `</html>`
